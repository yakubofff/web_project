...
function CheckAnswers() {
  ...
  fetch(url, {
    ...
    body: JSON.stringify({
iam: iam0 * 1 + iam1 * 2 + iam2 * 3 + iam3 * 4 + iam4 * 5
and_this: and_this1 * 1 + and_this2 * 2 + and_this3 * 3 + and_this * 4
gun_weigh: gun_weight
...
omg: omg
who: who
who_touched_my_gun: who1 * 1 + who2 * 2 + who3 * 3 + who4 * 4
laugh = laugh1 * 1 + laugh2 * 2 + laugh3 * 3 + laugh4 * 4 + laugh5 * 5
    }),
  })
    .then((response) => response.text())
    .then((text) => JSON.parse(text))
